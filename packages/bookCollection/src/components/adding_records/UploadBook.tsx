...
const UploadBook: React.FC = () => {
	...
	const handleAuthorAdded = (target: {
		firstName: string;
		secondName: string;
		thirdName: string;
		lastName: string;
		...
		setNewAuthors(prev => {
			...
			const updatedList = prev.filter(author => {
				const isFirstAndLastMatch =
					author.firstName === target.firstName &&
					author.lastName === target.lastName;

				if (!isFirstAndLastMatch) return true;

				const isSecondThirdMatch =
					author.secondName === target.secondName &&
					author.thirdName === target.thirdName;

				return !isSecondThirdMatch;
			});
